...
/**
 * Set sensitivity in volts/100µÒg/m3
...
/**
 * Get already set sensor sensitivity in volts/100µg/m3
 */
...
/**
 * Get average dust density between numSamples in µg/m3
 * With the default value of numSamples (20) the reading should take 200ms
 * 
 * @return uint16_t dust density between 0 and 600 µg/m3
 */
uint16_t GP2YDustSensor::getDustDensity(uint16_t numSamples)
{
    ...
    float scaledVoltage = avgRaw * (3.3 / 4095.0) * calibrationFactor;
    ...
    if (scaledVoltage < zeroDustVoltage) {
        ...
        // dustDensity is expressed in µg/m3
        dustDensity = (scaledVoltage - zeroDustVoltage) * 100 / this->sensitivity;
    }
 ...
 * 
 * @return uint16_t average dust density value between 0 and 600 µg/m3
 * 
 */
